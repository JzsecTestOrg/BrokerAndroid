...
def login_smoking(self, phone, password):
    ...
    try:
        ...
        globalData.LOG += generateLog.format_log('取消更新')
    except:
        globalData.LOG += generateLog.format_log('已是最新版本，无需更新！')
...
def login(self, i):
    ...
    if(Data.getTestdata('login', i, 9) != '请输入手机号' and Data.getPrecondition('login', i) != '手机号未注册'):
    ...
    try:
        if(Data.getTestdata('login', i, 4) == 'None'):
            ...
            if(loginElements.cipherpasswordText(self) != None):
            ...
            if(loginElements.plainpasswordText(self) != None):
